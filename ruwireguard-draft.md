...
Настоящий документ содержит начальное описание протокола безопасности сетевого уровня WireGuard с российскими криптографическими алгоритмами.
...
Пусть P обозначает инкапсулированный IP-пакет, отправляемый стороной m стороне m'. Тогда значения полей receiver, counter, packet формируются следующим образом:
```
P := P || 0^Z, Z = 16 ⌈|P| / 16⌉ – |P|
msg.counter := Nm-send
...
3c 14 18 6c 57 49 54 fa 26 52 2f 50 ee 3b 2f 8b
```
